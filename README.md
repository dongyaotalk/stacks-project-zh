...
段落和展示数学组成，或带一个 `enumerate` 且枚举前后各一段自然语言的单个
definition；以及同样由普通段落和展示数学组成，或带一个 `enumerate` 且枚举前后各一段
自然语言，并可带相邻简单 proof 的单个 lemma。它保护行内/展示数学、锁定引用命令、
TeX 控制空格和旧式 `{\it ...}` 外壳；遇到范围外的
