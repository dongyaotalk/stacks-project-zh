...
翻译以忠于锁定原文为准，不承担修复英文原文的责任。发现原文 typo、缺失证明或
论证疑点时，在 GitHub Issue 或非正文元数据中以 `remark` 留档即可；不补证明、
不改公式，也不把原文问题作为翻译待办或单独的候选/审校/发布阻塞项。译文自身的
错误及既有结构、术语、人工审校和发布门禁仍须处理。详见 `docs/translation-rules.md`。
...
- 术语符合已批准词表，待决术语已经解决；
- 独立批评中没有未关闭的翻译或流程 blocker/critical 问题；忠实保留的原文问题
  按 remark 记录，不归入翻译修复；
- 已完成要求的人工语言和数学审校；
- 生成的 TeX 可以在目标书稿中编译；
