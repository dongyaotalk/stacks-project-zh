...
| 优先级 | 定义 | 章数 |
| --- | --- | ---: |
| P0（最高优先级） | 中文资源稀缺且位于 Stacks Project 核心主线，应首先推进 | 24 |
| P1（高优先级） | 直接支撑核心主线，或具有很高的研究与教学价值 | 34 |
| P2（中优先级） | 重要但替代教材较多，或属于第二阶段进阶内容 | 31 |
| P3（低优先级） | 受众较窄或技术性较强，按明确读者需求推进 | 23 |
| P4（暂缓） | 维护、废弃、许可或自动生成内容，不作为正文翻译投入重点 | 5 |
...
| P0 | 第 98 章 Artin 公理（`artin`） | Section 1 / `07T0` | `UNPREPARED` | `PREPARE_SCOPE` |

## 全部 117 章

| 优先级 | wave / order | 章 | 轨道 | 当前下一范围 | 下一动作 | 原因 |
| --- | ---: | --- | --- | --- | --- | --- |
| P0 | 1 / 1 | 第 105 章 代数栈导论（`stacks-introduction`） | `entry`, `stacks-moduli` | Section 1 / `072I` | `PREPARE_SCOPE` | 作者设计的快速入口，建议作为首个新范围推进 |
| P0 | 1 / 2 | 第 1 章 引言（`introduction`） | `entry` | Section 1 / `0001` | `REVIEW` | 项目导航，篇幅短且能快速建立全书路线 |
| P0 | 1 / 3 | 第 7 章 位点与层（`sites`） | `scheme-to-stack` | Section 1 / `00V0` | `PREPARE_SCOPE` | 位点与层是下降和栈理论的核心语言 |
| P0 | 1 / 4 | 第 8 章 栈（`stacks`） | `scheme-to-stack` | Section 1 / `0267` | `PREPARE_SCOPE` | 进入代数栈的直接概念前置 |
| P0 | 1 / 5 | 第 26 章 概形（`schemes`） | `scheme-to-stack` | Section 1 / `01H9` | `PREPARE_SCOPE` | 整个代数几何主干和后续栈理论的基础 |
| P0 | 1 / 6 | 第 28 章 概形的性质（`properties`） | `scheme-to-stack` | Section 1 / `01OI` | `PREPARE_SCOPE` | 概形性质是高频参考核心 |
| P0 | 1 / 7 | 第 29 章 概形的态射（`morphisms`） | `scheme-to-stack` | Section 1 / `01QM` | `PREPARE_SCOPE` | 概形态射是高频参考核心 |
| P0 | 1 / 8 | 第 34 章 概形上的拓扑（`topologies`） | `scheme-to-stack`, `etale-cohomology` | Section 1 / `020L` | `PREPARE_SCOPE` | étale、fppf、fpqc 主线的共同基础 |
| P0 | 1 / 9 | 第 35 章 下降（`descent`） | `scheme-to-stack` | Section 1 / `0239` | `PREPARE_SCOPE` | 下降是 Stacks Project 最具中文化边际价值的核心章节之一 |
| P0 | 1 / 10 | 第 41 章 概形的平展态射（`etale`） | `scheme-to-stack`, `etale-cohomology` | Section 1 / `024K` | `PREPARE_SCOPE` | 概形的 étale 态射是 étale 几何主干 |
| P0 | 1 / 11 | 第 65 章 代数空间（`spaces`） | `scheme-to-stack` | Section 1 / `025S` | `PREPARE_SCOPE` | 代数空间是从概形到栈的关键桥梁 |
| P0 | 1 / 12 | 第 74 章 下降与代数空间（`spaces-descent`） | `scheme-to-stack` | Section 1 / `03YF` | `PREPARE_SCOPE` | 下降与代数空间是核心桥梁章节 |
| P0 | 1 / 13 | 第 94 章 代数栈（`algebraic`） | `stacks-moduli` | Section 1 / `026L` | `PREPARE_SCOPE` | 代数栈是项目核心 |
| P0 | 1 / 14 | 第 95 章 栈的例子（`examples-stacks`） | `entry`, `stacks-moduli` | Section 1 / `04SM` | `PREPARE_SCOPE` | 栈的例子具有极高的学习价值 |
| P0 | 1 / 15 | 第 97 章 可表性判据（`criteria`） | `representability`, `stacks-moduli` | Section 1 / `05XF` | `PREPARE_SCOPE` | 可表性判据是模问题核心 |
...
| P1 | 1 / 34 | 第 112 章 文献指南（`guide`） | `reference` | Section 1 / `03B1` | `REVIEW` | 翻译成本较低且研究导航价值高 |
...
| P4 | 1 / 1 | 第 113 章 期望事项（`desirables`） | `maintenance` | Section 1 / `02B5` | `REVIEW` | 项目维护愿望清单，不是核心教材正文 |
| P4 | 1 / 2 | 第 114 章 编码风格（`coding`） | `maintenance` | Section 1 / `02BZ` | `REVIEW` | 仅服务项目贡献者的编码规范 |
| P4 | 1 / 3 | 第 115 章 已废弃内容（`obsolete`） | `maintenance` | Section 1 / `073U` | `REVIEW` | 废弃历史内容，不值得继续优先投入 |
| P4 | 1 / 4 | 第 116 章 GNU 自由文档许可证（`fdl`） | `maintenance` | Section 1 / `05BG` | `PREPARE_SCOPE` | 许可证应按规范处理，不作为数学正文翻译任务 |
| P4 | 1 / 5 | 第 117 章 自动生成索引（`index`） | `generated` | — | `NOT_APPLICABLE` | 自动生成索引，应由中文版重新生成 |
