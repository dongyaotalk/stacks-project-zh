...
无标签导言；
由普通段落和展示数学组成，或带一个 `enumerate` 的单个 definition；以及同样由普通
段落和展示数学组成，或带一个 `enumerate`，并可带相邻简单 proof 的单个 lemma。枚举
前须有一段自然语言，枚举项可包含普通段落和展示数学，枚举后至多有一段自然语言。它
依据锁定的 `tags/tags` 生成稳定 Tag unit，保护行内/展示数学、锁定引用命令、TeX 控制
空格、可翻译脚注外壳以及旧式 `{\it ...}`、`{\em ...}` 外壳，并在遇到范围外的嵌套
标签、未知文本命令、环境
或 TeX 注释时失败。其他定义、引理、列表、表格或嵌套结构仍须等待解析器扩展和对应
测试，不得退回手工创建 unit。
